...
def _run_api_check_semantic(file_path, mode, name, expr_str, params=None, hints=["check_heading_always_on_grid"], timeout_ms=60_000, property_scope="all", optimization_level=OptimizationLevel.NONE, pc_target=None):
    ...
    if params is not None:
        params_str = str({':' + k: v for k, v in params.items()})
    with redirect_stdout(StringIO()):
        return CHC_Verification_semantic(
            file_name=file_path,
            mode=mode,
            user_properties=[UserProperty(name, expr_str)],
            params=params_str,
            property_scope=property_scope,
            hints=hints,
            timeout_ms=timeout_ms,
...
class ChironTestCase(unittest.TestCase):
    ...
    def return_from_api_check(self, name: str, expr: str):
        ...
        return _run_api_check(self._file_path, self.MODE, name, expr, self._params, self._hints, self._timeout_ms, self._property_scope, self._optimization_level, self._pc_target)

    def return_from_semantic_api_check(self, name: str, expr: str):
        """Run CHC_Verification_semantic and return the full result object for inspection."""
        return _run_api_check_semantic(self._file_path, self.MODE, name, expr, self._params, self._hints, self._timeout_ms, self._property_scope, self._optimization_level, self._pc_target)
    ...
    def _assert_semantic_property_status(self, name: str, expr: str, expected: str, heading_grid_expected: str = None):
        ...
        self.assertEqual(
            result.status, expected,
            f"Semantic property '{name}': expected {expected}, got {result.status}",
        )
        if heading_grid_expected is not None:
            self.assertEqual(
                result.heading_grid_safe, heading_grid_expected,
                f"Semantic property '{name}': expected heading grid to be {heading_grid_expected}, got {result.heading_grid_safe}",
            )
        return result
    ...
    def assert_semantic_property_pass(self, name: str, expr: str):
        """Assert helper-style semantic property is an invariant (PASSED)."""
        return self._assert_semantic_property_status(name, expr, "PASSED", heading_grid_expected="PASSED")

    def assert_semantic_property_fail(self, name: str, expr: str):
        """Assert helper-style semantic property is NOT an invariant (FAILED)."""
        return self._assert_semantic_property_status(name, expr, "FAILED", heading_grid_expected="PASSED")

    def assert_semantic_property_unknown(self, name: str, expr: str):
        """Assert helper-style semantic property status is UNKNOWN."""
        return self._assert_semantic_property_status(name, expr, "UNKNOWN", heading_grid_expected="PASSED")
