...
\begin{itemize}[leftmargin=1.5em]
	\item \hyperref[subsec:callable_api]{\textbf{Pipeline refactoring into a callable API:}}
	...
	\item \hyperref[subsec:heading_grid_safety]{\textbf{Heading-grid safety checks integrated:}}
	...
	\item \hyperref[subsec:property_scope]{\textbf{Property scope feature added:}}
	...
	\item \hyperref[subsec:heading_grid_hints]{\textbf{Heading-grid hints integrated:}}
	...
	\item \hyperref[subsec:opt_pipeline]{\textbf{Optimization pipeline added:}}
	...
	\item \hyperref[subsec:correctness_tests]{\textbf{Correctness tests:}}
	An expanded test suite now covers a wider variety of programs and properties, 
	including terminating-state properties, tests with heading-grid hints, and 
	some tests with \texttt{OptimizationLevel.BASIC} to validate correctness under optimization.

	\item \hyperref[sec:performance_tests]{\textbf{Performance tests:}} A performance test suite was designed to evaluate the impact of optimizations and hints on verification time and completion rates across a large set of programs.
\end{itemize}
...
\tikzset{
	...
	gate/.style={draw, rounded corners=2.5mm, align=left, text width=4.0cm, minimum height=2.25cm, fill=orange!14}
}
...
\subsection*{Implementation Details: Callable Verification API}\label{subsec:callable_api}
...
\subsection*{Implementation Details: Heading-grid Safety Check}\label{subsec:heading_grid_safety}
\textit{
	Files:
	\begin{enumerate}
...
\subsection*{Implementation Details: Property Scope}\label{subsec:property_scope}
\noindent\textit{\small File: \texttt{CHC-Verification/safety\_properties.py}}
...
\subsection*{Implementation Details: Optimization Pipeline}\label{subsec:opt_pipeline}
...
An additional semantic tightening is that movement trigonometric handling was upgraded
...
See \autoref{fig:opt_workflow} for a visual overview of the optimization flow.


\subsection*{Implementation Details: Heading-grid Hints}\label{subsec:heading_grid_hints}
\noindent\textit{\small File: \texttt{CHC-Verification/safety\_properties.py}}
...
\subsection*{API Usage Guide}\label{subsec:api_usage_guide}
